...
from utils import bert_base_AdamW_LLRD
...
def get_logger_and_args(logger_name: str, _args: List[str] = None):
    ...
    # ===== Get logger =====
    logger = get_logger(
        logger_name, exp_dir=train_args.logging_dir, rank=train_args.local_rank
    )
...
def generate_testing_results(
    train_args, logger, predictions, test_dataset, title, for_nested_ner=False
):
    ...
    if not for_nested_ner:
        ...
        pred_entities1 = extract_entities(
            predictions[:, 1:, 0], for_nested_ner=True, first_labels=True
        )
        pred_entities2 = extract_entities(
            predictions[:, 1:, 1], for_nested_ner=True, first_labels=False
        )
...
def main(_args: List[str] = None):
    ...
    # ===== Trainer =====
    compute_metrics = (
        ComputeMetricsForNestedNER() if for_nested_ner else ComputeMetricsForNER()
    )
    ...
    trainer = Trainer(
        ...
        tokenizer=tokenizer,
        args=train_args,
        data_collator=CollateFnForEE(
            tokenizer.pad_token_id, for_nested_ner=for_nested_ner
        ),
    ...
    if train_args.do_predict:
        ...
        predictions, _labels, _metrics = trainer.predict(
        ...
        generate_testing_results(
            ...
            predictions,
            test_dataset,
            title="CMeEE_test.json",
            for_nested_ner=for_nested_ner,
        )

        predictions, _labels, _metrics = trainer.predict(
            dev_dataset, metric_key_prefix="val"
        )
        generate_testing_results(
            train_args,
            logger,
